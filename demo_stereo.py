...
p1rect = cv.stereo.tform_pts(p1, H1)
p2rect = cv.stereo.tform_pts(p2, H2)
...
pl.plot(p1rect[0,:], p1rect[1,:], color=colors['red'], marker='.', ls='None')
pl.axis('off')
pl.xlim([0, nx-1])
pl.ylim([ny-1, 0])
...
pl.plot(p2rect[0,:], p2rect[1,:], color=colors['red'], marker='.', ls='None')
pl.axis('off')
pl.xlim([0, nx-1])
pl.ylim([ny-1, 0])
